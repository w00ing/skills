...
- Keep product, brand, platform, file-format, and technical names in their official spelling, following the terminology supplied for the project.
- Common examples include `Mac`, `macOS`, `PDF`, `ZIP`, `MP4`, `JPG`, `PNG`, `WebP`, `OCR`, and `API`.
- Prefer familiar Mainland Chinese terms where they fit the product and context:
